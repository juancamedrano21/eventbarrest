...
it('lists vendors and creates one landing on its profile', function (): void {
    $this->actingAs($this->owner)
        ->get('/event-panel/comercios')
        ->assertOk()
        ->assertSee('Tacos del Puerto');

    $this->actingAs($this->owner)
        ->post('/event-panel/comercios', [
            'name' => 'La Cervecería',
            'rnc' => '1-31-24680-9',
            'contact_name' => 'Pedro',
        ])
...
it('lists events and creates one landing on its detail', function (): void {
    $this->actingAs($this->owner)
        ->get('/event-panel/eventos')
        ->assertOk()
        ->assertSee('Bocao 2026');

    $this->actingAs($this->owner)
        ->post('/event-panel/eventos', [
            'name' => 'Bocao 2027',
            'venue' => 'Malecón',
...
it('shows the event detail with participants and outlets', function (): void {
    ...
    $this->actingAs($this->owner)
        ->get("/event-panel/eventos/{$this->event->id}")
        ->assertOk()
        ->assertSee('Tacos del Puerto')
        ->assertSee('Barra')
        ->assertSee('10.00');
...
it('keeps vendor staff out of every hito 2 screen', function (): void {
    ...
    $this->actingAs($staff)->get('/event-panel/comercios')->assertForbidden();
});
...
it('never shows another tenants event', function (): void {
    ...
    $this->actingAs($this->owner)->get("/event-panel/eventos/{$ajeno->id}")->assertNotFound();
});
