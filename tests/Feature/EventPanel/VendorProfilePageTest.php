...
it('never shows a vendor from another account', function (): void {
    ...
    $this->actingAs($this->owner)
        ->get("/event-panel/comercios/{$ajeno->id}")
        ->assertNotFound();
});

it('creates vendor staff from the profile form', function (): void {
    $this->actingAs($this->owner)
        ->post("/event-panel/comercios/{$this->vendor->id}/usuarios", [
            'name' => 'María',
            'username' => 'Maria',
...
it('invites the vendor to an event with its commission', function (): void {
    $this->actingAs($this->owner)
        ->post("/event-panel/comercios/{$this->vendor->id}/invitar", [
...
it('creates an outlet already attached to the vendor', function (): void {
    ...
    $this->actingAs($this->owner)
        ->post("/event-panel/comercios/{$this->vendor->id}/puestos", [
            'event_id' => $this->event->id,
...
it('saves the configuration: logo, business type and food type', function (): void {
    ...
    $this->actingAs($this->owner)
        ->post("/event-panel/comercios/{$this->vendor->id}/datos", [
...
it('builds the menu: category classified as Bebidas and its product', function (): void {
    ...
    $this->actingAs($this->owner)
        ->post("/event-panel/comercios/{$this->vendor->id}/categorias", [
    ...
    $this->actingAs($this->owner)
        ->post("/event-panel/comercios/{$this->vendor->id}/productos", [
            'name' => 'Cuba Libre',
            'price' => '450.50',
...
it('lets the owner reprice and deactivate a vendor product, never a foreign one', function (): void {
    ...
    $this->actingAs($this->owner)
        ->post("/event-panel/comercios/{$this->vendor->id}/productos/{$producto->id}", [
            'price' => '275', 'active' => 0,
        ])
        ->assertRedirect();
    ...
    $this->actingAs($this->owner)
        ->post("/event-panel/comercios/{$this->vendor->id}/productos/{$ajeno->id}", ['price' => '1'])
        ->assertNotFound();
});
...
it('lets the owner stock the vendor through the ledger', function (): void {
    ...
    $this->actingAs($this->owner)
        ->post("/event-panel/comercios/{$this->vendor->id}/insumos", [
    ...
    $this->actingAs($this->owner)
        ->post("/event-panel/comercios/{$this->vendor->id}/compras", [
...
it('still keeps vendor staff away from the owner operations', function (): void {
    ...
    $this->actingAs($staff)
        ->post("/event-panel/comercios/{$this->vendor->id}/productos", ['name' => 'X', 'price' => '1', 'category_id' => 1])
        ->assertForbidden();
});
...
it('links a simple product to an item and builds a recipe from the profile', function (): void {
    ...
    $this->actingAs($this->owner)
        ->post("/event-panel/comercios/{$this->vendor->id}/insumos", ['name' => 'Presidente (unidad)', 'base_unit' => 'unidad'])
        ->assertRedirect();
    $this->actingAs($this->owner)
        ->post("/event-panel/comercios/{$this->vendor->id}/insumos", ['name' => 'Ron', 'base_unit' => 'ml'])
        ->assertRedirect();
    ...
    $this->actingAs($this->owner)
        ->post("/event-panel/comercios/{$this->vendor->id}/categorias", ['name' => 'Bar', 'tipo' => 'bebidas'])
        ->assertRedirect();
    ...
    $this->actingAs($this->owner)
        ->post("/event-panel/comercios/{$this->vendor->id}/productos/{$receta->id}/receta", [
    ...
    // Y se puede quitar.
    $this->actingAs($this->owner)
        ->post("/event-panel/comercios/{$this->vendor->id}/productos/{$receta->id}/receta/{$ingrediente->id}/eliminar")
        ->assertRedirect();
...
it('creates an itbis exempt product and re-taxes it from its row', function (): void {
    ...
    $this->actingAs($this->owner)
        ->post("/event-panel/comercios/{$this->vendor->id}/productos/{$agua->id}", ['itbis_exempt' => 0])
        ->assertRedirect();
...
it('refuses a duplicate product name within the vendor', function (): void {
    ...
    $this->actingAs($this->owner)
        ...
        ->post("/event-panel/comercios/{$this->vendor->id}/productos/{$dos->id}", [
            'name' => 'Taco',
        ])
...
it('404s foreign categories and items from the modal and shields recipes from direct links', function (): void {
    ...
    $this->actingAs($this->owner)
        ->post("/event-panel/comercios/{$this->vendor->id}/productos/{$simple->id}", ['category_id' => $catAjena->id])
        ->assertNotFound();

    $this->actingAs($this->owner)
        ->post("/event-panel/comercios/{$this->vendor->id}/productos/{$simple->id}", ['inventory_item_id' => $itemAjeno->id])
        ->assertNotFound();

    // Una receta descuenta por su escandallo: el vínculo directo se ignora.
    $this->actingAs($this->owner)
        ->post("/event-panel/comercios/{$this->vendor->id}/productos/{$receta->id}", ['inventory_item_id' => $ronPropio->id])
        ->assertRedirect();
...
it('logs who changed a price and shows it in the summary', function (): void {
    ...
    $this->actingAs($this->owner)
        ->post("/event-panel/comercios/{$this->vendor->id}/productos/{$producto->id}", ['price' => '275'])
        ->assertRedirect();
