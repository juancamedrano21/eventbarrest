...
@section('content')
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse ($events as $event)
                    <tr class="hover:bg-gray-50">
                        <td class="px-5 py-3">
                            <a href="{{ route('event-panel.events.show', $event) }}" class="font-medium text-gray-800 hover:text-sky-600">{{ $event->name }}</a>
                        ...
                        <td class="px-5 py-3 text-right">
                            <a href="{{ route('event-panel.events.show', $event) }}" class="text-sm text-sky-600 hover:text-sky-700">Ver →</a>
                        </td>
                    </tr>
                @empty
    ...
    <div id="modal-evento" class="hs-overlay hidden size-full fixed top-0 start-0 z-80 overflow-y-auto" role="dialog" tabindex="-1">
        <div class="m-3 mt-14 sm:mx-auto sm:w-full sm:max-w-md">
            <form method="POST" action="{{ route('event-panel.events.store') }}" class="rounded-xl border border-gray-200 bg-white p-5 shadow-xl">
                @csrf
