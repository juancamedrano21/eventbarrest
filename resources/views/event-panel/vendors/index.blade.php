...
@section('content')
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse ($vendors as $vendor)
                    <tr class="hover:bg-gray-50">
                        <td class="px-5 py-3">
                            <a href="{{ route('event-panel.vendors.show', $vendor) }}" class="font-medium text-gray-800 hover:text-sky-600">{{ $vendor->name }}</a>
                        ...
                        <td class="px-5 py-3 text-right">
                            <a href="{{ route('event-panel.vendors.show', $vendor) }}" class="text-sm text-sky-600 hover:text-sky-700">Entrar →</a>
                        </td>
                    </tr>
                @empty
    ...
    {{-- Modal: nuevo negocio --}}
    <div id="modal-comercio" class="hs-overlay hidden size-full fixed top-0 start-0 z-80 overflow-y-auto" role="dialog" tabindex="-1">
        <div class="m-3 mt-14 sm:mx-auto sm:w-full sm:max-w-md">
            <form method="POST" action="{{ route('event-panel.vendors.store') }}" class="rounded-xl border border-gray-200 bg-white p-5 shadow-xl">
                @csrf
