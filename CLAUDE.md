...
## Los dos repos, y cómo se rompen entre sí

| Repo | Qué es |
|---|---|
| **`eventbarrest`** (este) | El backend Laravel, todos los frontends web **y la documentación** (`docs/`). Es el repo principal: aquí vive la memoria del proyecto |
| `../../payrone-table-kds/` | La app Android del KDS (Kotlin + WebView). Repo propio |

La documentación **viaja con el código**, en este repo, para que no se queden
desincronizados: quien clona `eventbarrest` tiene el sistema y su porqué en el mismo
sitio. El repo Android es aparte porque es otro toolchain, pero **lee la documentación
de aquí** (ver su `CLAUDE.md`).

**No son independientes.** El APK es un cascarón de ~700 líneas: **la pantalla que
enseña la sirve este repo**. Un cambio aquí puede dejar la tablet inútil sin que nada
falle en el navegador ni en los tests, y sin que nadie se entere hasta el festival.
...
| `resources/js/kds/**` | Corre **dentro del WebView de la tablet**, no solo en el navegador. `npm run build` aquí cambia lo que ejecuta el APK sin tocar su repo. Y el WebView es más viejo que Chrome de escritorio: por eso se evitó `AbortSignal.timeout` y se usa `AbortController` a mano |
...
**Al terminar un cambio en cualquiera de los dos, el CHANGELOG que se actualiza es
siempre el de aquí** (`docs/CHANGELOG.md`): es la memoria común. El APK usa el área
`APK (tablet)`.
...
```bash
./vendor/bin/pint --dirty && ./vendor/bin/phpstan analyse --no-progress --memory-limit=1G && php artisan test
```
...
Commitea cuando el trabajo esté verde, en el repo que toque (código y documentación van juntos aquí; el APK aparte). **No empujes salvo que se
